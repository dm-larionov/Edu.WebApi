...
using Edu.WebApi.Application.Auditing;
using Edu.WebApi.Application.Identity.Users;
using Edu.WebApi.Application.Identity.Users.Password;

namespace Edu.WebApi.Host.Controllers.Personal;
...
public class PersonalController : VersionNeutralApiController
{
    ...
    [ApiConventionMethod(typeof(EduApiConventions), nameof(EduApiConventions.Register))]
    ...
    [HttpGet("roles")]
    [OpenApiOperation("Get roles of currently logged in user.", "")]
    public async Task<ActionResult<List<UserRoleDto>>> GetRolesAsync(CancellationToken cancellationToken)
    {
        return User.GetUserId() is not { } userId || string.IsNullOrEmpty(userId)
            ? Unauthorized()
            : Ok(await _userService.GetRolesAsync(userId, cancellationToken));
    }
